...
class Swarm:
    ...
    def __init__(self, smiles, x, v, x_min=-10., x_max=10.,
        ...
        """
        #From MoLeR 2022 paper.
        #MoLeR’s encoder is regularized through the Lprior loss term. 
        #Concretely, the mean of the distribution predicted by the encoder is penalized proportionally to its norm. 
        # This suggests that a ball may better approximate the encoder’s distribution than a hypercube, which we indeed found to hold in practice. 
        # Therefore, for MoLeR we clip to a ball of fixed radius R = 10
        # in python not sure if this is [latent_vector] = 10 * [latent_vector] or using clip method
        self.smiles = smiles
        self.x = x
        self.v = v
        self.x_min = x_min
    ...
    def from_query(cls, init_sml, init_emb, num_part, v_min=-0.6, v_max=0.6, *args, **kwargs):
        ...
        v = np.random.uniform(v_min, v_max, [num_part, init_emb[0].shape[0]])
        # old code v = np.random.uniform(v_min, v_max, [num_part, init_emb.shape[-1]])
